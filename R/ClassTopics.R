...
#'                regarding the algorithm steps (default = \code{TRUE})
#' 
#' @return rstan model to interpret with the function ClassTopics_results
#' 
#' @details
...
ClassTopics <- function(counts,
  ...
  if(verbose){
    topic_corr <- rstan::extract(fit, "topic_correlations")$topic_correlations
    mean_corr <- apply(topic_corr, c(2,3), mean)
...
predict_ClassTopics_stan <- function(
  ...
  mod <- .stan_source_path("model_test")
  ...
  response_class <- rstan::extract(fit, "y_pred")$y_pred
  predicted_class <- apply(response_class, 2, function(x) {
    as.numeric(names(sort(table(x), decreasing = TRUE))[1])
  })
  
  response_probs <- rstan::extract(fit, "response_probs")$response_probs
  predicted_probs <- apply(response_probs, c(2, 3), mean)
...
.cv_ClassTopics_fold_by_fold <- function(
  ...
  W_samples_pred <- rstan::extract(fit_fold, "W")$W
  W_pred <- apply(W_samples_pred, c(2, 3), mean)
  
  eta_samples <- rstan::extract(fit_fold, "eta")$eta
  eta_uncentered_fold <- matrix(apply(eta_samples, 1, mean), C, K_topics)
  eta_means_mat <- matrix(colMeans(eta_uncentered_fold),
                          nrow = C, ncol = K_topics, byrow = T)
  eta_fold <- eta_uncentered_fold - eta_means_mat
  
  y_pred_samples <- rstan::extract(fit_fold, "y_pred")$y_pred
  y_pred_mode <- apply(y_pred_samples, 2, function(x) {
    as.numeric(names(sort(table(x), decreasing = TRUE))[1])
  })
...
ClassTopics_results <- function(fit, true_response,
  ...
  # Extract parameters
  theta_samples <- rstan::extract(fit, "theta")$theta
  beta_samples <- rstan::extract(fit, "beta")$beta
  
  # Get dimensions
  n_iter <- dim(theta_samples)[1]
  D <- dim(theta_samples)[2]
  K <- dim(theta_samples)[3]
  ...
  eta_samples <- rstan::extract(fit, "eta")$eta
  eta_uncentered <- matrix(apply(eta_samples, 1, mean), C, K)
  eta_means_mat <- matrix(colMeans(eta_uncentered),
                          nrow = C, ncol = K, byrow = T)
  
  # Patient-topic distributions
  theta_mean <- apply(theta_samples, c(2, 3), mean)
  theta_lower <- apply(theta_samples, c(2, 3), quantile, probs = alpha_level)
  theta_upper <- apply(theta_samples, c(2, 3), quantile, probs = 1 - alpha_level)
  ...
  beta_lower <- apply(beta_samples, c(2, 3), quantile, probs = alpha_level)
  beta_upper <- apply(beta_samples, c(2, 3), quantile, probs = 1 - alpha_level)
  ...
  eta_mean <- eta_uncentered - eta_means_mat
  eta_lower <- matrix(apply(eta_samples, 1, quantile, probs = alpha_level),
                      C, K) - eta_means_mat
  eta_upper <- matrix(apply(eta_samples, 1, quantile, probs = 1 - alpha_level),
                      C, K) - eta_means_mat
  
  # Extract predictions
  y_pred_samples <- rstan::extract(fit, "y_pred")$y_pred
  y_pred_mode <- apply(y_pred_samples, 2, function(x) {
    as.numeric(names(sort(table(x), decreasing = TRUE))[1])
  })
  
  response_probs <- rstan::extract(fit, "response_probs")$response_probs
  response_probs_mean <- apply(response_probs, c(2, 3), mean)
  ...
  # Topic correlations
  cors_samples <- rstan::extract(fit, "topic_correlations")$topic_correlations
  topic_cors <- apply(cors_samples, c(2, 3), mean)
